# gateway. Built from the repository root:
#
#   docker build -f deploy/docker/gateway.Dockerfile -t surge/gateway .
#
...
RUN --mount=type=cache,target=/go/pkg/mod \
    ...
    CGO_ENABLED=1 GOOS=linux go build -trimpath -ldflags="-s -w" -o /out/gateway ./services/gateway/cmd
