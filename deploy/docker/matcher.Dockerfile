# matcher. Built from the repository root:
#
#   docker build -f deploy/docker/matcher.Dockerfile -t surge/matcher .
#
...
RUN --mount=type=cache,target=/go/pkg/mod \
    ...
    CGO_ENABLED=1 GOOS=linux go build -trimpath -ldflags="-s -w" -o /out/matcher ./services/matcher/cmd
