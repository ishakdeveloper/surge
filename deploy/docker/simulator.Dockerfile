# simulator. Built from the repository root:
#
#   docker build -f deploy/docker/simulator.Dockerfile -t surge/simulator .
#
...
RUN --mount=type=cache,target=/go/pkg/mod \
    ...
    CGO_ENABLED=1 GOOS=linux go build -trimpath -ldflags="-s -w" -o /out/simulator ./services/simulator/cmd
