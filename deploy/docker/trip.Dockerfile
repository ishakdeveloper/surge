# trip. Built from the repository root:
#
#   docker build -f deploy/docker/trip.Dockerfile -t surge/trip .
#
...
RUN --mount=type=cache,target=/go/pkg/mod \
    ...
    CGO_ENABLED=1 GOOS=linux go build -trimpath -ldflags="-s -w" -o /out/trip ./services/trip/cmd
...
COPY --from=build /out/trip /app/trip
