...
stripe-listen: ## Forward Stripe webhooks to the local gateway
	@set -a; . ./.env; set +a; stripe listen --api-key "$$STRIPE_SECRET_KEY" \
		--events setup_intent.succeeded,payment_intent.amount_capturable_updated,payment_intent.payment_failed,payment_intent.succeeded,payment_intent.canceled,payout.paid,payout.failed,payout.canceled \
