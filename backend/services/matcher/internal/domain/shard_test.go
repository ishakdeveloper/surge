...
func must(t *testing.T, shard *domain.Shard, event wire.GeoEvent, now time.Time) domain.Outcome {
	t.Helper()
...
func TestCheckpointCarriesPromisesAndNotPositions(t *testing.T) {
	losing := domain.NewShard(0, domain.DefaultConfig())
	...
	gaining := domain.NewShard(0, domain.DefaultConfig())
...
func TestRestoredHoldExpiresIfTheDriverNeverReturns(t *testing.T) {
	...
	losing := domain.NewShard(0, config)
	must(t, losing, entered(t, "drv-1", nearby, 1), base)
	must(t, losing, request(t, "trip-a", dam), base)

	gaining := domain.NewShard(0, config)
