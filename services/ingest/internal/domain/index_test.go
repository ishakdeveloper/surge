...
import (
	...
	"github.com/ishakdeveloper/surge/ingest/internal/service"
...
func TestTranslateShardCrossingProducesAPair(t *testing.T) {
	...
	entry := service.Translate(first, time.Now())
	...
	events := service.Translate(second, time.Now())
	if len(events) != 2 {
...
func TestTranslateWithinShardIsOneMove(t *testing.T) {
	...
	_ = service.Translate(first, time.Now())
...
func TestTranslateDropsStale(t *testing.T) {
	if events := service.Translate(domain.Observation{Stale: true}, time.Now()); events != nil {
